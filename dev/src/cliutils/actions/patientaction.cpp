...
bool PatientAction::run(const QStringList &iargs)
{
    ...
    SharedPatient p;
    Tucuxi::GuiCore::Response r = ADMINREPO->getPatientFromId(id, p);
    if (r.error != NoError) {
        return false;
    }
...
//        return false;
    Tucuxi::GuiCore::Response r = ADMINREPO->getPatientsList(list);
...
//        return false;
    ...
    if (r.error != Tucuxi::GuiCore::NoError) {
        return false;
    }
...
//        return false;
    Tucuxi::GuiCore::Response r = ADMINREPO->getMeasuresByDate(patient->id(), list);
    if (r.error != NoError && r.error != NotFound) {
        return false;
    }
    //Print it
...
//                     tr("Error while retrieving the patient's covariate list from the database :")) )
    Tucuxi::GuiCore::Response r = ADMINREPO->getPatientVariatesByDate(patient->id(),list);
    if (r.error != NoError && r.error != NotFound) {
        return false;
    }
        ...
        if (existing == false) {
            if (!CORE->definitionManager()->covariateExists(args.at(1)) && APPCORE->drugManager()->drug(drugid) == 0) {
                ...
                return false;
                    ...
                    c = Tucuxi::GuiCore::CoreFactory::createEntity<PatientVariate>( ADMINREPO);
