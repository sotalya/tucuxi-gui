...
int RequestsClientProcessing::analyzeList(const QString &xmlList, QString &controlId)
{
    ...
    for (int i = 0; i < requestsContent.size(); ++i) {
        ...
        if (drugId.isEmpty())
        {
            EXLOG(QtWarningMsg, ezechiel::core::NOEZERROR, tr("Cannot build interpretationRequest from Pending Request. The drug id %1 is unknown. Aborting this request.").arg(restDrugId));
            continue;
        }
        ...
        if (!request->drug()) {
            ...
            EXLOG(QtWarningMsg, ezechiel::core::NOEZERROR, tr("Cannot build interpretationRequest from Pending Request. Aborting this request."));
            rlutil::resetColor();
            continue;
        } else {
        ...
        if (!request->drug()) {
            EXLOG(QtWarningMsg, ezechiel::core::NOEZERROR, tr("Cannot build interpretationRequest from Pending Request. Aborting."));
            continue;
            return 0;
        }
...
InterpretationRequest* SimpleBuilder::buildRequest(const QString &xmlRequest)
{
    ...
    if (!ir) {
        EXLOG(QtWarningMsg, ezechiel::core::NOEZERROR, tr("Could not build interpretationRequest from Pending Request. Aborting."));
        return nullptr;
    }
