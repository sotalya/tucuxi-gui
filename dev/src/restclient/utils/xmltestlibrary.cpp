    ...
    _requestIds()
{
    ...
    //Load the files
    loadTestFile(_queryListTestFile, ezechiel::core::XmlValidator::Queries, _queryList);
    loadTestFile(_replyListTestFile, ezechiel::core::XmlValidator::Reply_List, _replyList);
    for (int i = 0; i < _queryRequestsTestFiles.size(); ++i)
        loadTestFile(_queryRequestsTestFiles.at(i), ezechiel::core::XmlValidator::Queries, _queryRequests);
    for (int i = 0; i < _replyRequestsTestFiles.size(); ++i)
        loadTestFile(_replyRequestsTestFiles.at(i), ezechiel::core::XmlValidator::Reply_Request, _replyRequests);
    for (int i = 0; i < _notificationsTestFiles.size(); ++i)
        loadTestFile(_notificationsTestFiles.at(i), ezechiel::core::XmlValidator::Notification, _notifications);
}
