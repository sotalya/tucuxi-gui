...
void TargetListModel::setModelData(QList<ezechiel::core::Target*> *targets)
{
    beginResetModel();
    _targets = targets;
    foreach (ezechiel::core::Target* tar, *targets) {
...
void TargetListModel::setModelData(ezechiel::core::DrugModel* drug)
{
    _drug = drug;
    emit updateStatus();
}
...
void TargetListModel::setDataImpl(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    case TypeRole:
        switch (value.toInt()) {
        case 0:
            _targets->at(row)->getType()->setTargetType(ezechiel::core::TargetMethod::ResidualTarget);
            break;
        case 1:
            _targets->at(row)->getType()->setTargetType(ezechiel::core::TargetMethod::PeakTarget);
            break;
        case 2:
            _targets->at(row)->getType()->setTargetType(ezechiel::core::TargetMethod::MeanTarget);
            break;
        default:
            _targets->at(row)->getType()->setTargetType(ezechiel::core::TargetMethod::UnknownTarget);
            break;
        }
        break;
        return;
...
void TargetListModel::insertRowsImpl(int at, int count, const ezechiel::core::SharedEntitySet &entities)
{
    ...
    for(int i = at, j = 0; i < at + count; i++, j++) {
        ...
        if (entities.isEmpty()) {
            ...
            target->getType()->setTargetType(ezechiel::core::TargetMethod::ResidualTarget);
