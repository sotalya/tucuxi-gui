...
#define TRAININGFILE_OPTION "1_trainingfile"
#define TESTFILE_OPTION     "2_testfile"
#define RANSAC_THRESHOLD    "3_threshold"
...
//	inlinerP = ransac(p, threshold, k);
    ...
    // Create a new patient
    {
        ...
        createPatient(&p, 0);
        p.num = 1000;
        p.sex = (int)parameters.get("sex").value;
        p.age = (int)parameters.get("age").value;
        p.weight = (int)parameters.get("weight").value;
        ++dbtest.size;
        dbtest.patients = (Patient *)realloc(dbtest.patients, dbtest.size * sizeof(Patient));
        dbtest.patients[dbtest.size - 1] = p;
...
//	testFeat(:,i) = (testFeat(:,i)-m1)/s1;
    ...
    int nbInliners = ransac(x, y, nbSamplesTrain, threshold, 6, alpha, inliners);
...
//	gsl_vector * out = gsl_vector_calloc(nbSamplesTest);
    ...
    for (int j = 0; j < nbInliners; ++j)
        gsl_vector_set(y2, j, y[inliners[j]]);
...
const int SvmPredict::_priority = 90;
...
SvmPredict::SvmPredict()
{
    ...
    _parameters.append(Descriptor("sex", tr("Sex"), tr("Sex of the patient")));
    _parameters.append(Descriptor("age", tr("Age"), tr("Current age of the patient")));
    _parameters.append(Descriptor("weight", tr("Weight"), tr("Current weight of the patient")));
    ...
    addOption(TRAININGFILE_OPTION, tr("Training data"), tr("File containing the training data"),
              qMetaTypeId<FileName>(), QVariant::fromValue(FileName(QCoreApplication::applicationDirPath() + "/" + DB_TRAIN_PATH, "*.txt")));

    //Name of the file containing the test data
    addOption(TESTFILE_OPTION, tr("Test data"), tr("File containing the test data"),
              qMetaTypeId<FileName>(), QVariant::fromValue(FileName(QCoreApplication::applicationDirPath() + "/" + DB_TEST_PATH, "*.txt")));

    //Name of the file containing the test data
    addOption(RANSAC_THRESHOLD, tr("RANSAC Threshold"), tr("Random Sample Consensus (RANSAC) algorithm threshold parameter"),
              QVariant::Double, QVariant(500.0));
}
...
DataSet SvmPredict::predict(const Duration &start, const Duration &until, const ParameterSet &parameters, uint points, const Dosage &dosage, Residuals &/*residuals*/)
{
    ...
    double threshold = option(RANSAC_THRESHOLD).value.toDouble();

    if (threshold < 0.0)
        threshold = 0.0;

    QByteArray trainingArray = trainingFile.toLocal8Bit();
    QByteArray testArray = testFile.toLocal8Bit();

    //Result
    DataSet data(points,_ugl);

    gsl_vector * out = gsl_vector_calloc(points);
    mainSvm(trainingArray.data(),
            testArray.data(),
            start.toHours(),
            until.toHours(),
            points,
