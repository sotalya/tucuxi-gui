...
qmake ${TUCUXI_SRC_DIR}/ezechiel.pro -spec macx-clang CONFIG+=x86_64 CONFIG+=qml_debug DEFINES+=NOLICENSE CONFIG+=qtquickcompiler CONFIG+=config_tucucore CONFIG+=config_tucucore_full CONFIG+=config_pure_tucucore DRUGSPATH=${TUCUXI_DRUGS_DIR} && /usr/bin/make qmake_all

# Build Tucuxi
make -j8

export BUNDLE=${TUCUXI_BUILD_DIR}/dist/ezechiel.app
export TARGET_BINARY=${BUNDLE}/Contents/MacOS/ezechiel
...
cp -r ${TUCUXI_DRUGS_DIR} ${BUNDLE}/Contents/Resources/drugs2
