    ...
    _requestIds()
{
    ...
    //Load the files
    loadTestFile(_queryListTestFile, Tucuxi::Gui::Core::XmlValidator::Queries, _queryList);
    loadTestFile(_replyListTestFile, Tucuxi::Gui::Core::XmlValidator::Reply_List, _replyList);
    for (int i = 0; i < _queryRequestsTestFiles.size(); ++i)
        loadTestFile(_queryRequestsTestFiles.at(i), Tucuxi::Gui::Core::XmlValidator::Queries, _queryRequests);
    for (int i = 0; i < _replyRequestsTestFiles.size(); ++i)
        loadTestFile(_replyRequestsTestFiles.at(i), Tucuxi::Gui::Core::XmlValidator::Reply_Request, _replyRequests);
    for (int i = 0; i < _notificationsTestFiles.size(); ++i)
        loadTestFile(_notificationsTestFiles.at(i), Tucuxi::Gui::Core::XmlValidator::Notification, _notifications);
}
